...
export const CartProvider = ({ children }) => {
  ...
  const getTotalDiscount = () => {
    return (
      cartData.cart.reduce(
        (acc, { price, qty }) => acc + Number(price) * qty, 0) - getTotalPrice()
    );
  };
  ...
  return (
    <CartContext.Provider
      value={{ addFilterCategory, addFilterRating, addFilterRange, addFilterSortby, addFilterQuery,
        clearFilter, clearCategory, addToCart, addToWishList, removeFromCart, removeFromWishlist,
        removeMultipleFromCart, resetCartContext, getCartCount, clearCart, toggleWishlist,
        getWishlistCount, isProductInCart, isProductInWishlist, updateQuantityCart, getTotalPrice,
        getTotalDiscount,
        range: cartData.filter.price, filter: cartData.filter, cart: cartData.cart,
        wishlist: cartData.wishlist, searchQuery: cartData.filter.searchQuery,
      }}>
      {children}
    </CartContext.Provider>
  );
